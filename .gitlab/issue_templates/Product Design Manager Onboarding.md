...
If you have questions, don't hesitate to @mention your UX buddy, @vkarnes (your Manager), or the whole UX department by using `@gitlab\\-com/gitlab\\-ux` (use the latter sparingly unless it's something important). You can also reach out to people in the `#ux` Slack channel.
...
* [ ] You should have now received an invite for the `Social calls` (every Tuesday and Thursday). If you can't attend the call, don't worry! There is no set agenda, just a time set aside for everyone to openly talk. 
