...
Dir["{hosted}/**/index.html"].each do |file|
  ...
  open('public/index.html', 'a+') { |f|
    url = URI.escape(dir)
    html = '<li class="html"><i class="fa fa-arrows" aria-hidden="true"></i><a href="' + url + '/">' + dir + '</a></li>'
    f.puts html
  }
end
...
## Old way of tracking HTML files (specs and HTML previews). Now anything in hosted that has an index.html will be tracked
## and added to the Public Folder. No need to suffix the folders with 'spec-preview' or "html-preview"
