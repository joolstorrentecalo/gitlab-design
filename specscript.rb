...
Dir["{hosted}/**/*"].grep(/index.html$/).each do |webdir|
  next unless File.directory?(webdir)
  ...
  open('public/index.html', 'a+') { |f|
    url = URI.escape(webdir)
    html = '<li class="html"><i class="fa fa-arrows" aria-hidden="true"></i><a href="' + url + '/">' + webdir + '</a></li>'
    f.puts html
  }
end  
